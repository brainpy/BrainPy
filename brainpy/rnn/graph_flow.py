...
def find_entries_and_exits(nodes, ff_edges, fb_edges=()):
  ...
  if len(lonely):
    raise ValueError(f"Found lonely nodes {lonely} which do not connect with any other.")

  # get input and output nodes
  entry_points = (ff_senders | fb_senders) - ff_receivers
  end_points = ff_receivers - ff_senders
...
def has_path(from_node, to_node, edges, method='dfs'):
  graph = defaultdict(list)
  for s, r in edges:

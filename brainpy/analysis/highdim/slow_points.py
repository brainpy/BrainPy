...
class SlowPointFinder(base.DSAnalyzer):
    ...
    def find_fps_with_gd_method(
        ...
        fixed_points = jax.tree.map(lambda a: bm.TrainVar(a), candidates, is_leaf=lambda x: isinstance(x, bm.Array))
    ...
    def find_fps_with_opt_solver(
        ...
        if self._opt_functions.get(F_OPT_SOLVER, None) is None:
            self._opt_functions[F_OPT_SOLVER] = self._get_f_for_opt_solver(candidates,
                                                                           SUPPORTED_OPT_SOLVERS[opt_solver])
        ...
        # optimizing
        res = f_opt(jax.tree.map(lambda a: bm.as_jax(a), candidates, is_leaf=lambda a: isinstance(a, bm.Array)))
    ...
    def compute_jacobians(
        ...
        info = np.asarray([(l.ndim, l.shape[0])
                           for l in jax.tree.flatten(points, is_leaf=lambda a: isinstance(a, bm.Array))[0]])

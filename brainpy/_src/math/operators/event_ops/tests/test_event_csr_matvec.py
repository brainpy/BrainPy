...
class Test_event_csr_matvec(parameterized.TestCase):
  ...
  def test_homo(self, shape, transpose, homo_data):
    ...
    r1 = event_csr_matvec(homo_data, indices, indptr, events,
                                     shape=shape, transpose=transpose)
    r2 = event_csr_matvec(heter_data, indices, indptr, events,
                                     shape=shape, transpose=transpose)
    self.assertTrue(jnp.allclose(r1, r2))

    r3 = cusparse_csr_matvec(homo_data, indices, indptr, events.astype(float),
                                        shape=shape, transpose=transpose)
    ...
    r5 = event_csr_matvec(heter_data, indices, indptr, events.astype(float),
                                     shape=shape, transpose=transpose)
  ...
  def test_homo_vamp(self, shape, transpose, homo_data):
    ...
    f1 = jax.vmap(partial(event_csr_matvec, indices=indices, indptr=indptr, events=events,
                          shape=shape, transpose=transpose))
    f2 = jax.vmap(partial(cusparse_csr_matvec, indices=indices, indptr=indptr, vector=events.astype(float),
                          shape=shape, transpose=transpose))
    vmap_data = bm.as_jax([homo_data] * 10)
    self.assertTrue(jnp.allclose(f1(vmap_data), f2(vmap_data)))

    # vmap 'events'
    f3 = jax.vmap(partial(event_csr_matvec, homo_data, indices, indptr,
                          shape=shape, transpose=transpose))
    f4 = jax.vmap(partial(cusparse_csr_matvec, homo_data, indices, indptr,
                          shape=shape, transpose=transpose))
    ...
    f5 = jax.vmap(lambda dd, ee: event_csr_matvec(dd, indices, indptr, ee,
                                                             shape=shape, transpose=transpose))
    f6 = jax.vmap(lambda dd, ee: cusparse_csr_matvec(dd, indices, indptr, ee,
                                                                shape=shape, transpose=transpose))
  ...
  def test_homo_grad(self, shape, transpose, homo_data):
    ...
    indptr = bm.as_jax(indptr)
    events = bm.as_jax(rng.random(shape[0] if transpose else shape[1])) < 0.1
    dense_conn = csr_to_dense(bm.ones(indices.shape).value, indices, indptr, shape=shape)

    # grad 'data'
    r1 = jax.grad(sum_op(event_csr_matvec))(
      homo_data, indices, indptr, events, shape=shape, transpose=transpose)
    r2 = jax.grad(sum_op(cusparse_csr_matvec))(
    ...
    # grad 'events'
    r4 = jax.grad(sum_op(event_csr_matvec), argnums=3)(
      homo_data, indices, indptr, events.astype(float), shape=shape, transpose=transpose)
    r5 = jax.grad(sum_op(cusparse_csr_matvec), argnums=3)(
      homo_data, indices, indptr, events.astype(float), shape=shape, transpose=transpose)
  ...
  def test_heter(self, shape, transpose):
    ...
    r1 = event_csr_matvec(heter_data, indices, indptr, events,
                                     shape=shape, transpose=transpose)
    r2 = cusparse_csr_matvec(heter_data, indices, indptr, events.astype(float),
                                        shape=shape, transpose=transpose)
    self.assertTrue(jnp.allclose(r1, r2))
    ...
    r4 = event_csr_matvec(heter_data, indices, indptr, events.astype(float),
                                     shape=shape, transpose=transpose)
    self.assertTrue(jnp.allclose(r1, r4))
  ...
  def test_heter_vamp(self, shape, transpose):
    ...
    f3 = jax.vmap(partial(event_csr_matvec, data, indices, indptr,
                          shape=shape, transpose=transpose))
    f4 = jax.vmap(partial(cusparse_csr_matvec, data, indices, indptr,
                          shape=shape, transpose=transpose))
    vmap_data = bm.as_jax(rng.random((10, shape[0] if transpose else shape[1]))) < 0.1
    self.assertTrue(jnp.allclose(f3(vmap_data), f4(vmap_data.astype(float))))
  ...
  def test_heter_grad(self, shape, transpose):
    ...
    dense_data = csr_to_dense(data, indices, indptr, shape=shape)
    r3 = jax.grad(sum_op(lambda a: ((events @ a) if transpose else
                                    (a @ events))))(dense_data)
    rows, cols = csr_to_coo(indices, indptr)
    r3 = r3[rows, cols]
    self.assertTrue(jnp.allclose(r1, r3))

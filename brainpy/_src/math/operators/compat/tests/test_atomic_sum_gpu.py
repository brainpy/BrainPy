...
from brainpy._src.math.operators.compat import coo_atomic_sum
...
class TestAtomicSum(unittest.TestCase):
  ...
  def test_homo_value1(self):
    ...
    value = 2.
    post_ids = jnp.arange(size, dtype=jnp.uint32)
    a = coo_atomic_sum(value, post_ids, size)
    print(a)
  ...
  def test_homo_fixedpro(self):
    ...
    a = coo_atomic_sum(value, bp.math.as_jax(post_ids), size)
    print(a)
  ...
  def test_heter_fixedpro(self):
    ...
    a = coo_atomic_sum(value, bp.math.as_jax(post_ids), size, bp.math.as_jax(pre_ids))
    print(a)

...
@ti.kernel
def event_csr_matvec_cpu(values: ti.types.ndarray(),
                         indices: ti.types.ndarray(),
                         indptr: ti.types.ndarray(),
                         events: ti.types.ndarray(),
                         transpose: ti.types.ndarray(),
...
event_csr_matvec_p = XLACustomOp(cpu_kernel=event_csr_matvec_cpu)
...
def csrmv(
  ...
  *,
  ...
  # computing
  return event_csr_matvec_p(data, indices, indptr, events, transpose, is_event_type_bool, is_heter, outs=[jax.ShapeDtypeStruct(shape=(shape[1] if transpose else shape[0],), dtype=data.dtype)])

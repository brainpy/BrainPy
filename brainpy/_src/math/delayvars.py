...
class DelayVariable(AbstractDelay):
  """Delay variable which has a fixed delay length.
  ...
  def __init__(
      ...
      target: Variable,
      length: int = 0,
      initial_delay_data: Union[float, int, bool, Array, jax.Array, Callable] = None,
      update_method: str = ROTATE_UPDATE
  ):
    super().__init__()
  ...
  def register_entry(
    ...
    if delay_time is not None:
      ...
      if callable(delay_time):
        delay_time = _as_jax_array(delay_time(self.delay_target_shape))
        delay_step = jnp.asarray(delay_time / get_dt(), dtype=get_int())
      elif isinstance(delay_time, float):
        delay_step = int(delay_time / get_dt())
      else:
        delay_step = jnp.asarray(_as_jax_array(delay_time) / get_dt(), dtype=get_int())
  ...
  def at_entry(self, entry: str, *indices) -> Array:
    ...
    if entry not in self._access_to_step:
      raise KeyError(f'Does not find delay access "{entry}".')
  ...
  def retrieve(self, delay_step, *indices):
    ...
    if self.update_method == ROTATE_UPDATE:
      delay_idx = (self.idx.value + delay_step) % self.length
      delay_idx = stop_gradient(delay_idx)
  ...
  def _init_data(self, length):
    ...
    # update delay data
    self.data[0] = self.target.value
    if isinstance(self._initial_delay_data, (Array, jax.Array, float, int, bool)):

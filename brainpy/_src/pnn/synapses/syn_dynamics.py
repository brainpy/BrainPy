...
from brainpy._src.pnn.mixin import ParamDesc, AlignPost
from brainpy._src.pnn.neurons.base import PNeuGroup, pneu_doc
...
class PSynDyn(PNeuGroup, ParamDesc):
  """Parallelizable synaptic dynamics."""
  pass


class Exponential(PSynDyn, AlignPost):
  ...
  **Model Descriptions**
  ...
  def reset_state(self, batch_size=None):
    self.g = self.sharding_variable(bm.zeros, batch_size)
...
Exponential.__doc__ = Exponential.__doc__ % (pneu_doc,)


class DualExponential(PSynDyn):
  ...
  **Model Descriptions**
  ...
  def __init__(
    ...
    super(DualExponential, self).__init__(name=name,
                                          mode=mode,
                                          size=size,
                                          keep_size=keep_size,
                                          axis_names=axis_names)
    ...
    # parameters
    self.tau_rise = self.sharding_param(tau_rise)
    self.tau_decay = self.sharding_param(tau_decay)
  ...
  def reset_state(self, batch_size=None):
    self.h = self.sharding_variable(bm.zeros, batch_size)
    self.g = self.sharding_variable(bm.zeros, batch_size)
...
DualExponential.__doc__ = DualExponential.__doc__ % (pneu_doc,)
...
class NMDA(PSynDyn):
  ...
  **Model Examples**
  ...
  def __init__(
    ...
    super(NMDA, self).__init__(name=name,
                               mode=mode,
                               size=size,
                               keep_size=keep_size,
                               axis_names=axis_names)
  ...
  def reset_state(self, batch_size=None):
    self.g = self.sharding_variable(bm.zeros, batch_size)
    self.x = self.sharding_variable(bm.zeros, batch_size)
...
class STD(PSynDyn):
  ...
  def reset_state(self, batch_size=None):
    self.x = self.sharding_variable(bm.ones, batch_size)
...
class STP(PSynDyn):
  ...
  def __init__(
    ...
    # parameters
    self.tau_f = self.sharding_param(tau_f)
    self.tau_d = self.sharding_param(tau_d)
    self.U = self.sharding_param(U)
    self.method = method
  ...
  def reset_state(self, batch_size=None):
    self.x = self.sharding_variable(bm.ones, batch_size)
    self.u = self.sharding_variable(bm.ones, batch_size)
    self.u.fill_(self.U)
...
class AMPA(PSynDyn):
  ...
  **Model Examples**
  ...
  def __init__(
    ...
    super(AMPA, self).__init__(name=name,
                               mode=mode,
                               size=size,
                               keep_size=keep_size,
                               axis_names=axis_names)
    ...
    # parameters
    self.alpha = self.sharding_param(alpha)
    self.beta = self.sharding_param(beta)
    self.T = self.sharding_param(T)
    self.T_duration = self.sharding_param(T_dur)
  ...
  def reset_state(self, batch_size=None):
    self.g = self.sharding_variable(bm.zeros, batch_size)
    self.spike_arrival_time = self.sharding_variable(bm.ones, batch_size)
...
class GABAa(AMPA):
  ...
  **Model Descriptions**
  ...
  def __init__(
    ...
    super(GABAa, self).__init__(alpha=alpha,
                                beta=beta,
                                T=T,
                                T_dur=T_dur,
                                method=method,
...
class BioNMDA(PSynDyn):
  ...
  **Model Descriptions**
  ...
  def __init__(
    ...
    super(BioNMDA, self).__init__(name=name,
                                  mode=mode,
                                  size=size,
                                  keep_size=keep_size,
                                  axis_names=axis_names)
    ...
    # parameters
    self.beta1 = self.sharding_param(beta1)
    self.beta2 = self.sharding_param(beta2)
    self.alpha1 = self.sharding_param(alpha1)
    self.alpha2 = self.sharding_param(alpha2)
    self.T = self.sharding_param(T)
    self.T_dur = self.sharding_param(T_dur)

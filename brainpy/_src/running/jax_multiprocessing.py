...
def jax_vectorize_map(
  ...
  elements, tree = tree_flatten(arguments, is_leaf=lambda a: isinstance(a, bm.BaseArray))
  if clear_buffer:
  ...
  for i in range(0, num_pars[0], num_parallel):
    ...
    if isinstance(arguments, dict):
      ...
      raise TypeError
    res_values, res_tree = tree_flatten(r, is_leaf=lambda a: isinstance(a, bm.BaseArray))
    if results is None:

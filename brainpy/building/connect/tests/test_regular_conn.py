...
def test_one2one():
  for size in [100, (3, 4), (4, 5, 6)]:
    ...
    conn.require('pre_ids', 'post_ids', 'pre2post', 'pre2syn', 'post2pre', 'post2syn')
    ...
    actual_mat = bp.math.asarray(actual_mat, dtype=np.float32)
...
def test_all2all():
  for has_self in [True, False]:
    for size in [100, (3, 4), (4, 5, 6)]:
      ...
      mat = conn.require(connect.CONN_MAT)
      conn.require('conn_mat', 'weight_mat', 'pre_ids', 'post_ids', 'pre2post', 'pre2syn', 'post2pre', 'post2syn')
      num = bp.tools.size2num(size)

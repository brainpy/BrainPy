...
def _check_connections(conn):
    if not isinstance(conn, (np.ndarray, math.JaxArray, jnp.ndarray, csr_matrix)):
...
class TwoEndConnector(Connector):
    ...
    def make_return(self, mat=None, ij=None, structures=None):
        ...
        if mat is not None:
            ...
            self._data = csr_matrix(mat, dtype=np.bool_)
            self._dense_mat = math.asarray(self._data.todense(), dtype=MAT_DTYPE)
        if ij is not None:
            _check_connections(ij[0])
            _check_connections(ij[1])
            data = np.ones_like(ij[0], dtype=np.bool_)
            self._data = csr_matrix((data, (ij[0], ij[1])), shape=(self.pre_num, self.post_num), dtype=np.bool_)
        # if sparse_mat is not None:
        #     _check_connections(sparse_mat)
        #     self._data = sparse_mat

        if len(self.structures) == 1 and self.structures[0] == CONN_MAT:
            if mat is None:
                mat = self._data.todense()
                self._dense_mat = math.asarray(mat, dtype=MAT_DTYPE)
            ...
            if CONN_MAT in self.structures:
                ...
                self._dense_mat = all_data[CONN_MAT]

...
class EinsumTest(jtu.JaxTestCase):
    ...
    def _check(self, s, *ops):
        a = np.einsum(s, *ops)
        b = bm.einsum(s, *ops, precision=lax.Precision.HIGHEST)
    ...
    def test_from_dask(self, einstr, dtype):
        r = jtu.rand_default(bm.random.RandomState())
        if '->' in einstr:
    ...
    def test_einsum_path(self):
        ...
        a = bm.random.RandomState().rand(2, 2)
        b = bm.random.RandomState().rand(2, 5)
        c = bm.random.RandomState().rand(5, 2)
        ...
        I = bm.random.RandomState().rand(10, 10, 10, 10)
        C = bm.random.RandomState().rand(10, 10)
    ...
    def test_no_unnecessary_transpose(self):
        ...
        x = r.randn(2, 2, 2)
        y = r.randn(2, 2)
        jaxpr = jax.make_jaxpr(partial(bm.einsum, "ijk,kl->ijl"))(x, y)

...
class ndarray(object):
  ...
  def argmax(self, axis=None):
    return ndarray(self.value.argmax(axis=axis))

  def argmin(self, axis=None):
    return ndarray(self.value.argmin(axis=axis))
  ...
  def argsort(self, axis=-1, kind=None, order=None):
    return ndarray(self.value.argsort(axis=axis, kind=kind, order=order))

  def astype(self, dtype, order='K', casting='unsafe', subok=True, copy=True):
    return ndarray(self.value.astype(dtype=dtype, order=order, casting=casting, subok=subok, copy=copy))

  def byteswap(self, inplace=False):
    return ndarray(self.value.byteswap(inplace=inplace))

  def choose(self, choices, mode='raise'):
    choices = choices.value if isinstance(choices, ndarray) else choices
    return ndarray(self.value.choose(choices=choices, mode=mode))

  def clip(self, min=None, max=None, **kwargs):
    return ndarray(self.value.clip(min=min, max=max, **kwargs))

  def compress(self, condition, axis=None):
    condition = condition.value if isinstance(condition, ndarray) else condition
    return ndarray(self.value.compress(condition=condition, axis=axis))

  def conj(self):
    return ndarray(self.value.conj())

  def conjugate(self):
    return ndarray(self.value.conjugate())

  def copy(self, order='C'):
    return ndarray(self.value.copy(order=order))

  def cumprod(self, axis=None, dtype=None):
    return ndarray(self.value.cumprod(axis=axis, dtype=dtype))

  def cumsum(self, axis=None, dtype=None):
    return ndarray(self.value.cumsum(axis=axis, dtype=dtype))

  def diagonal(self, offset=0, axis1=0, axis2=1):
    return ndarray(self.value.diagonal(offset=offset, axis1=axis1, axis2=axis2))

  def dot(self, b):
    return ndarray(self.value.dot(b))
  ...
  def flatten(self, order='C'):
    return ndarray(self.value.flatten(order=order))

  def getfield(self, dtype, offset=0):
    return ndarray(self.value.getfield(dtype=dtype, offset=offset))
  ...
  def newbyteorder(self, new_order='S', *args, **kwargs):
    return ndarray(self.value.newbyteorder(new_order=new_order, *args, **kwargs))

  def nonzero(self):
    return tuple(ndarray(a) for a in self.value.nonzero())

  def partition(self, kth, axis=-1, kind='introselect', order=None):
    return ndarray(self.value.partition(kth=kth, axis=axis, kind=kind, order=order))
  ...
  def ptp(self, axis=None, keepdims=False):
    return ndarray(self.value.ptp(axis=axis, keepdims=keepdims))

  def ravel(self, order=None):
    return ndarray(self.value.ravel(order=order))
  ...
  def sort(self, axis=-1, kind=None, order=None):
    return ndarray(self.value.sort(axis=axis, kind=kind, order=order))

  def size(self):
    return self.value.size()

  def split(self, indices_or_sections, axis=0):
    return [ndarray(a) for a in self.value.split(indices_or_sections, axis=axis)]

  def squeeze(self, axis=None):
    return ndarray(self.value.squeeze(axis=axis))

  def std(self, axis=None, dtype=None, ddof=0, keepdims=False, *args, **kwargs):
    return ndarray(self.value.std(axis=axis, dtype=dtype, ddof=ddof, keepdims=keepdims,
                                  *args, **kwargs))
  ...
  def swapaxes(self, axis1, axis2):
    return ndarray(self.value.swapaxes(axis1, axis2))

  def take(self, indices, axis=None, mode='raise'):
    indices = indices.value if isinstance(indices, ndarray) else indices
    return ndarray(self.value.take(indices=indices, axis=axis, mode=mode))

  def tobytes(self, order='C'):
    return ndarray(self.value.tobytes(order=order))

  def tile(self, reps):
    reps = reps.value if isinstance(reps, ndarray) else reps
    return ndarray(self.value.tile(reps))

  def tolist(self):
    return self.value.tolist()

  def trace(self, offset=0, axis1=0, axis2=1, dtype=None):
    return ndarray(self.value.trace(offset=offset, axis1=axis1, axis2=axis2, dtype=dtype))

  def transpose(self, *axes):
    return ndarray(self.value.transpose(*axes))

  def var(self, axis=None, dtype=None, ddof=0, keepdims=False, *args, **kwargs):
    return ndarray(self.value.var(axis=axis, dtype=dtype, ddof=ddof, keepdims=keepdims, *args, **kwargs))

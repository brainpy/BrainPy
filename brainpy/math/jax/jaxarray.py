...
class JaxArray(object):
  ...
  def __eq__(self, oc):
    return type(self)(self._value.__eq__(oc._value if isinstance(oc, JaxArray) else oc))

  def __ne__(self, oc):
    return type(self)(self._value.__ne__(oc._value if isinstance(oc, JaxArray) else oc))

  def __lt__(self, oc):
    return type(self)(self._value.__lt__(oc._value if isinstance(oc, JaxArray) else oc))

  def __le__(self, oc):
    return type(self)(self._value.__le__(oc._value if isinstance(oc, JaxArray) else oc))

  def __gt__(self, oc):
    return type(self)(self._value.__gt__(oc._value if isinstance(oc, JaxArray) else oc))

  def __ge__(self, oc):
    return type(self)(self._value.__ge__(oc._value if isinstance(oc, JaxArray) else oc))

  def __add__(self, oc):
    return type(self)(self._value.__add__(oc._value if isinstance(oc, JaxArray) else oc))

  def __radd__(self, oc):
    return type(self)(self._value.__radd__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __sub__(self, oc):
    return type(self)(self._value.__sub__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rsub__(self, oc):
    return type(self)(self._value.__rsub__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __mul__(self, oc):
    return type(self)(self._value.__mul__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rmul__(self, oc):
    return type(self)(self._value.__rmul__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __div__(self, oc):
    return type(self)(self._value.__div__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rdiv__(self, oc):
    return type(self)(self._value.__rdiv__(oc._value if isinstance(oc, JaxArray) else oc))

  def __truediv__(self, oc):
    return type(self)(self._value.__truediv__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rtruediv__(self, oc):
    return type(self)(self._value.__rtruediv__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __floordiv__(self, oc):
    return type(self)(self._value.__floordiv__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rfloordiv__(self, oc):
    return type(self)(self._value.__rfloordiv__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __divmod__(self, oc):
    return type(self)(self._value.__divmod__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rdivmod__(self, oc):
    return type(self)(self._value.__rdivmod__(oc._value if isinstance(oc, JaxArray) else oc))

  def __mod__(self, oc):
    return type(self)(self._value.__mod__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rmod__(self, oc):
    return type(self)(self._value.__rmod__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __pow__(self, oc):
    return type(self)(self._value.__pow__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rpow__(self, oc):
    return type(self)(self._value.__rpow__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __matmul__(self, oc):
    return type(self)(self._value.__matmul__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rmatmul__(self, oc):
    return type(self)(self._value.__rmatmul__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __and__(self, oc):
    return type(self)(self._value.__and__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rand__(self, oc):
    return type(self)(self._value.__rand__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __or__(self, oc):
    return type(self)(self._value.__or__(oc._value if isinstance(oc, JaxArray) else oc))

  def __ror__(self, oc):
    return type(self)(self._value.__ror__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __xor__(self, oc):
    return type(self)(self._value.__xor__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rxor__(self, oc):
    return type(self)(self._value.__rxor__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __lshift__(self, oc):
    return type(self)(self._value.__lshift__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rlshift__(self, oc):
    return type(self)(self._value.__rlshift__(oc._value if isinstance(oc, JaxArray) else oc))
  ...
  def __rshift__(self, oc):
    return type(self)(self._value.__rshift__(oc._value if isinstance(oc, JaxArray) else oc))

  def __rrshift__(self, oc):
    return type(self)(self._value.__rrshift__(oc._value if isinstance(oc, JaxArray) else oc))

...
def cross_entropy_loss(logits, targets, weight=None, reduction='mean'):
  ...
  output : scalar, mjax.JaxArray
    ...
    in the case of K-dimensional loss.
  """
  targets = targets.value if isinstance(targets, JaxArray) else targets
...
def l1_loos(logits, targets, reduction='sum'):
  ...
  diff = diff.value if isinstance(diff, JaxArray) else diff
  norm = jn.linalg.norm(diff, ord=1, axis=1, keepdims=False)
...
def l2_loss(predicts, targets):
  ...
  """
  r = 0.5 * (predicts - targets) ** 2
  return r.value if isinstance(r, JaxArray) else r
...
def mean_absolute_error(x, y, axis=None):
  ...
  """
  r = ops.abs(x - y)
  return jn.mean(r.value if isinstance(r, JaxArray) else r, axis=axis)
...
def log_cosh(predicts, targets=None, ):
  ...
  errors = errors.value if isinstance(errors, JaxArray) else errors

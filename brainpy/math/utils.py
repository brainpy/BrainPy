...
def every_to_step_num(interval):
  from brainpy.math import get_dt
  num_interval = round(interval / get_dt())
  if math.fmod(interval * 1000, get_dt() * 1000) != 0.:
    print(f'"{interval}" is not an integer multiple of the step '
          f'resolution ("{get_dt()}"). BrainPy adjust it '
          f'to "{num_interval * get_dt()}".')
  return num_interval

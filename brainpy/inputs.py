...
def period_input(values, durations, dt=None, return_length=False):
  ...
  # get the current
  start = 0
  I_current = ops.zeros((int(ops.ceil(I_duration / dt)),) + I_shape)
...
def spike_input(points, lengths, sizes, duration, dt=None):
  ...
  current = ops.zeros(int(ops.ceil(duration / dt)))
...
def ramp_input(c_start, c_end, duration, t_start=0, t_end=None, dt=None):
  ...
  current = ops.zeros(int(ops.ceil(duration / dt)))
  p1 = int(ops.ceil(t_start / dt))
  p2 = int(ops.ceil(t_end / dt))
  current[p1: p2] = ops.array(ops.linspace(c_start, c_end, p2 - p1))
  return current
...
class PoissonInput(NeuGroup):
  ...
  def update(self, _t, _i):
    self.spike = ops.random.random(self.num) <= self.freqs * self.dt

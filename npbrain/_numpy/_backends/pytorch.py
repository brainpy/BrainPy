...
from typing import Tuple, Any, Optional, TypeVar
...
class ndarray:
    ...
    def __bool__(self: ArrayType) -> ArrayType:
    ...
    @property
    def size(self: ArrayType) -> int:
        return self.data.nelement()

    @property
    def strides(self: ArrayType) -> int:
    ...
    def flatten(self: ArrayType) -> ArrayType:
    ...
    def reshape(self: ArrayType, shape: Shape) -> ArrayType:
...
def log2(arr: ArrayType) -> ArrayType:
    return ndarray(torch.log2(arr.data))


def log10(arr: ArrayType) -> ArrayType:
    return ndarray(torch.log10(arr.data))


def log1p(arr: ArrayType) -> ArrayType:
    return ndarray(torch.log1p(arr.data))


def matmul(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
...
def power(arr: ArrayType, exponent: ArrayOrScalar) -> ArrayType:
    exponent = exponent.data if isinstance(exponent, ndarray) else exponent
    return ndarray(torch.pow(arr.data, exponent))


def sign(arr: ArrayType) -> ArrayType:
...
def square(arr: ArrayType) -> ArrayType:
...
def arccos(arr: ArrayType) -> ArrayType:
    return ndarray(torch.acos(arr.data))


def arccosh(arr: ArrayType) -> ArrayType:
    return ndarray(torch.acosh(arr.data))


def arcsin(arr: ArrayType) -> ArrayType:
    return ndarray(torch.asin(arr.data))


def arcsinh(arr: ArrayType) -> ArrayType:
    return ndarray(torch.asinh(arr.data))


def arctan(arr: ArrayType) -> ArrayType:
    return ndarray(torch.atan(arr.data))


def arctan2(arr: ArrayType) -> ArrayType:
    return ndarray(torch.atan2(arr.data))


def arctanh(arr: ArrayType) -> ArrayType:
...
def cosh(arr: ArrayType) -> ArrayType:
    return ndarray(torch.cosh(arr.data))


def deg2rad(arr: ArrayType) -> ArrayType:
    return ndarray(torch.deg2rad(arr.data))


def radians(arr: ArrayType) -> ArrayType:
    return ndarray(torch.deg2rad(arr.data))


def rad2deg(arr: ArrayType) -> ArrayType:
    return ndarray(torch.rad2deg(arr.data))


def degrees(arr: ArrayType) -> ArrayType:
    return ndarray(torch.rad2deg(arr.data))


def hypot(arr: ArrayType) -> ArrayType:
...
def sinc(arr: ArrayType) -> ArrayType:
    return ndarray(torch.sin(arr.data) / arr.data)


def sinh(arr: ArrayType) -> ArrayType:
    return ndarray(torch.sinh(arr.data))


def tan(arr: ArrayType) -> ArrayType:
    return ndarray(torch.tan(arr.data))


def tanh(arr: ArrayType) -> ArrayType:
...
def bitwise_and(arr: ArrayType) -> ArrayType:
    return ndarray(torch.bitwise_and(arr.data))


def bitwise_not(arr: ArrayType) -> ArrayType:
    return ndarray(torch.bitwise_not(arr.data))


def invert(arr: ArrayType) -> ArrayType:
    return ndarray(torch.bitwise_not(arr.data))


def bitwise_or(arr: ArrayType) -> ArrayType:
    return ndarray(torch.bitwise_or(arr.data))


def bitwise_xor(arr: ArrayType) -> ArrayType:
...
def equal(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.eq(arr1.data, arr2.data))


def not_equal(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.ne(arr1.data, arr2.data))


def logical_and(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.logical_and(arr1.data, arr2.data))


def logical_or(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
...
def logical_xor(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.logical_xor(arr1.data, arr2.data))


def greater(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.gt(arr1.data, arr2.data))


def greater_equal(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.ge(arr1.data, arr2.data))


def less(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.lt(arr1.data, arr2.data))


def less_equal(arr1: ArrayType, arr2: ArrayType) -> ArrayType:
    return ndarray(torch.le(arr1.data, arr2.data))


def minimum(arr1: ArrayType, arr2: ArrayOrScalar) -> ArrayType:
...
def maximum(arr1: ArrayType, arr2: ArrayOrScalar) -> ArrayType:
    if isinstance(arr2, ndarray):
        arr2 = arr2.data
    elif isinstance(arr2, int) or isinstance(arr2, float):
        arr2 = torch.full_like(arr1.data, arr2)
...
def ceil(arr: ArrayType) -> ArrayType:
    return ndarray(torch.ceil(arr.data))


def floor(arr: ArrayType) -> ArrayType:
    return ndarray(torch.floor(arr.data))


def fmod(arr: ArrayType) -> ArrayType:
...
def isinf(arr: ArrayType) -> ArrayType:
    return ndarray(torch.isinf(arr.data))


def isnan(arr: ArrayType) -> ArrayType:
    return ndarray(torch.isnan(arr.data))


def trunc(arr: ArrayType) -> ArrayType:
...
def shape(arr: ArrayType) -> ArrayType:
    return arr.data.shape


def reshape(arr: ArrayType, shape: Shape) -> ArrayType:
    return ndarray(arr.data.reshape(shape))


def ravel(arr: ArrayType) -> ArrayType:
    # https://github.com/pytorch/pytorch/issues/1582
    return ndarray(arr.data.view(-1))


def flatten(arr: ArrayType, start: int = 0, end: int = -1) -> ArrayType:
    return ndarray(arr.flatten(start, end))


def moveaxis(arr: ArrayType, source: int, destination: int) -> ArrayType:
    # https://github.com/pytorch/pytorch/issues/36048
    return ndarray(torch.movedim(arr.data, source, destination))


def transpose(arr: ArrayType, axes: Optional[Axes] = None) -> ArrayType:
...
def split(arr: ArrayType, indices_or_sections: ArrayOrScalar, axis: int = 0) -> Tuple:
...
def tile(arr: ArrayType, multiples: Axes) -> ArrayType:
    return ndarray(arr.data.repeat(multiples))


def repeat(arr: ArrayType, repeats: ArrayOrScalar, axis: int = None):
...
def flip(arr: ArrayType, axis: Optional[AxisAxes] = None) -> ArrayType:
    if axis is None:
...
def copy(arr: ArrayOrScalar) -> ArrayType:
    return ndarray(arr.data.clone().detach())


def empty(shape: ShapeOrScalar, dtype: Any = None, **kwargs: Any) -> ArrayType:
    _check()
...
def empty_like(arr: ArrayType) -> ArrayType:
    return ndarray(torch.empty_like(arr.data))


def ones(shape: ShapeOrScalar, dtype: Any = None, **kwargs: Any) -> ArrayType:
    _check()
    shape = shape if isinstance(shape, Iterable) else (shape,)
    dtype = torch.float64 if dtype is None else dtype
...
def zeros(shape: ShapeOrScalar, dtype: Any = None, **kwargs: Any) -> ArrayType:
    _check()
    shape = shape if isinstance(shape, Iterable) else (shape,)
    dtype = torch.float64 if dtype is None else dtype
...
def linspace(start: int, stop: int, num: int = 50, dtype: Any = None, **kwargs: Any) -> ArrayType:
    _check()
...
def eye(N: int, M: int = None, dtype: Any = None, **kwargs: Any) -> ArrayType:
    _check()
    return ndarray(torch.eye(n=N, m=M, dtype=dtype, **kwargs))


def identity(N: int, dtype: Any = None, **kwargs: Any) -> ArrayType:
    _check()
    return ndarray(torch.eye(n=N, m=None, dtype=dtype, **kwargs))


def meshgrid(arr: ArrayType, *arrays: ArrayType, indexing: str = "xy") -> Tuple[ArrayType, ...]:
    arrays = tuple(arr.data if isinstance(arr, ndarray) else arr for arr in arrays)
...
def array(data: Any, dtype: Any = None, device: Any = None) -> ArrayType:
    _check()
    data = np.array(data)
    return ndarray(torch.as_tensor(data, dtype=dtype, device=device))


def asarray(data: Any, dtype: Any = None, device: Any = None) -> ArrayType:
    _check()
...
def uniform(low: float = 0.0, high: float = 1.0, size: ShapeOrScalar = 1, **kwargs: Any) -> ArrayType:
    _check()
...
def expand_dims(arr: ArrayType, axis: int) -> ArrayType:
    return ndarray(arr.data.unsqueeze(dim=axis))


def pad(arr: ArrayType, paddings: Tuple[Tuple[int, int], ...],
        mode: str = "constant", value: float = 0, ) -> ArrayType:
...
def where(arr: ArrayType, x: ArrayOrScalar, y: ArrayOrScalar) -> ArrayType:
...
def take_along_axis(arr: ArrayType, index: ArrayType, axis: int) -> ArrayType:
...
def all(t: ArrayType, axis: Optional[AxisAxes] = None, keepdims: bool = False) -> ArrayType:
    return t.all(axis=axis, keepdims=keepdims)


def any(t: ArrayType, axis: Optional[AxisAxes] = None, keepdims: bool = False) -> ArrayType:
    return t.any(axis=axis, keepdims=keepdims)


def argmin(t: ArrayType, axis: Optional[int] = None) -> ArrayType:
    return t.argmin(axis=axis)


def argmax(t: ArrayType, axis: Optional[int] = None) -> ArrayType:
    return t.argmax(axis=axis)


def argsort(t: ArrayType, axis: int = -1) -> ArrayType:
    return t.argsort(axis=axis)


def cumsum(t: ArrayType, axis: Optional[int] = None) -> ArrayType:
    return t.cumsum(axis=axis)


def clip(t: ArrayType, min_: float, max_: float) -> ArrayType:
    return t.clip(min_, max_)


def from_numpy(t: ArrayType, a: Any) -> ArrayType:
    return t.from_numpy(a)


def prod(t: ArrayType, axis: Optional[AxisAxes] = None, keepdims: bool = False) -> ArrayType:
    return t.prod(axis=axis, keepdims=keepdims)


def pow(t: ArrayType, exponent: ArrayOrScalar) -> ArrayType:
    return t.pow(exponent)


def squeeze(t: ArrayType, axis: Optional[AxisAxes] = None) -> ArrayType:
    return t.squeeze(axis=axis)


def sum(t: ArrayType, axis: Optional[AxisAxes] = None, keepdims: bool = False) -> ArrayType:
    return t.sum(axis=axis, keepdims=keepdims)


def sort(t: ArrayType, axis: int = -1) -> ArrayType:

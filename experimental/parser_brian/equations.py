...
from experimental.parser_brian import Expression
from experimental.parser_brian.sympytools import sympy_to_str, str_to_sympy
from experimental.parser_brian.stringtools import get_identifiers
...
# # FLAG = Combine(Word(alphas) + Optional(Word('= ') + Word('-.' + nums)))
# # FLAG = Word(alphas + nums + ',-.= ')
# FLAG = Word(alphas, alphas + nums + ',-.= ')
#
# # # Flags are comma-separated and enclosed in parantheses: ":flag1=1., flag2"
# # FLAGS = Group(Suppress(':') + FLAG + ZeroOrMore(Suppress(',') + FLAG)).setResultsName('flags')
# # FLAGS = Combine(Suppress(':') + Word(alphas + nums + ',-.= ')).setResultsName('flags')
# FLAGS = Group(Suppress(':') + FLAG).setResultsName('flags')
...
# Static equation:
# x = 2 * y : volt (flags)
STATIC_EQ = Group(IDENTIFIER + Suppress('=') + EXPRESSION +
                  Optional(FLAGS)).setResultsName(SUBEXPRESSION)

# Differential equation
# dx/dt = -x / tau : volt
DIFF_OP = (Suppress('d') + IDENTIFIER + Suppress('/') + Suppress('dt'))
DIFF_EQ = Group(DIFF_OP + Suppress('=') + EXPRESSION +
...
eqs = '''
dv/dt = (-v + a + 2*sin(2*pi*t/tau))/tau (volt)
a (volt)
'''

# eqs = 'dv/dt = (-v + a + 2*sin(2*pi*t/tau))/tau : volt'
...
class SingleEquation(Hashable):
    ...
    def __init__(self, type, varname, dimensions, var_type=FLOAT, expr=None, flags=None):
        self.type = type
...
class Equations(Hashable, Mapping):
        ...
        for eq in self.values():
            ...
            if eq.varname in replacements:
                ...
                if not isinstance(new_varname, str):
                    raise ValueError('Cannot replace model variable "%s" with a value' % eq.varname)
            ...
            if eq.type in [SUBEXPRESSION, DIFF_EQUATION]:
    ...
    identifier_checks = {check_identifier_basic,
    ...
    def get_substituted_expressions(self, variables=None,
        ...
        if include_subexpressions:
            ...
            return [(name, expr) for name, expr in self._substituted_expressions
                    if self[name].type == DIFF_EQUATION]

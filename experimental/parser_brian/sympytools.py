...
DEFAULT_CONSTANTS = {
    'pi': sympy.pi,
    'e': sympy.E,
    'inf': sympy.core.singleton.S.Infinity,
    '-inf': sympy.core.singleton.S.NegativeInfinity
}


class SympyNodeRenderer():
    expression_ops = {
        'Add': sympy.Add,
        'Mult': sympy.Mul,
        'Pow': sympy.Pow,
    ...
    def render_func(self, node):
        ...
        elif node.id in DEFAULT_FUNCTIONS:
            return DEFAULT_FUNCTIONS[node.id]
        else:
    ...
    def render_element_parentheses(self, node):
        ...
        elif node.__class__.__name__ in ['Num', 'Constant'] and getattr(node, 'n', getattr(node, 'value', None)) >= 0:
            return self.render_node(node)
...
def check_expression_for_multiple_stateful_functions(expr, variables):
    ...
    for identifier, count in identifier_count.items():
        ...
        if count > 1 and isinstance(var, sympy.Function) and not var.stateless:
            raise NotImplementedError(
                ('The expression "{expr}" contains more than one call of {func}, this '
                 'is currently not supported since {func} is a stateful function and '
                 'its multiple calls might be treated incorrectly (e.g. "rand() - rand()" '
                 'could be simplified to "0.0").').format(expr=expr, func=identifier))
...
def sympy_to_str(sympy_expr):
    ...
    replacements = dict((f, sympy.Function(name)) for name, f in DEFAULT_FUNCTIONS.items())
    # replace constants with our names as well
    replacements.update(dict((c, sympy.Symbol(name)) for name, c in DEFAULT_CONSTANTS.items()))
